...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _loadMessages() async {
    ...
    if (response != null && mounted) {
      ...
      _chatService.markAsRead(widget.complaintId);
    }
  }

  void _connectToChat() {
    final authService = Provider.of<AuthService>(context, listen: false);
    final officerId = authService.currentUser?['user_id'];
    
    if (officerId != null) {
      _chatService.connect(widget.complaintId, officerId, 'officer');
      ...
      _chatService.socket?.on('user_typing', (data) {
        if (data['user_type'] == 'user' && mounted) {
          setState(() => _isUserTyping = data['is_typing']);
        }
      });
    }
  }
  ...
  Future<void> _sendMessage() async {
    ...
    // Send via WebSocket
    _chatService.sendMessage(widget.complaintId, officerId, 'officer', message);
            ...
            child: _messages.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final isMe = message['sender_type'] == 'officer';
